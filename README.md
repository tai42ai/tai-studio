...
a `tai42-skeleton` MCP server. Browse and exercise tools, extensions, connectors,
templates, and human-in-the-loop interactions against a running skeleton,
straight from the browser. It is the platform's web UI, and the
...
> command**: it needs Docker, `uv`, the Playwright chromium browser, and the
> source checkouts its `*_DIR` environment variables point at. Rerun it after
> any UI or branding change. See `e2e/docs-demo/README.md`.
...
```bash
pnpm install
pnpm -r build                         # build first — see below
pnpm -r typecheck
pnpm -r lint
pnpm -r format:check
...
Those five checks are the `check` job of `.github/workflows/ci.yml`, in that
order. The build comes first because every package resolves the other
...
`tai42-skeleton` — its quickstart boots a working server, then point this
Studio at it.
