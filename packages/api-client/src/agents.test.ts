...
describe('streamAuthoredAgentRun', () => {
  ...
  it('POSTs the non-baked input to the authored-run path and yields parsed events', async () => {
    ...
    const gen = await streamAuthoredAgentRun({ ...config, fetch: fetchMock }, 'assistant_bot', {
    ...
    expect(knownTypes(events).at(-1)).toBe('stream.end');
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toMatch(/^\/api\/agents\/authored\/assistant_bot\/runs\?_=/);
    expect(init.method).toBe('POST');
    expect((init.headers as Record<string, string>)['x-api-key']).toBe('key');
    expect(init.body).toBe(JSON.stringify({ user_message: 'what is the status of my request?' }));
  });
  ...
  it('gives every open a DISTINCT url so concurrent runs are never coalesced', async () => {
    ...
    expect(urls).toHaveLength(2);
    expect(urls[0]).not.toBe(urls[1]);
    for (const url of urls) expect(url).toMatch(/^\/api\/agents\/authored\/assistant_bot\/runs\?/);
  });
  ...
  it('surfaces the baked-field override 400 message verbatim (not the bare status text)', async () => {
    ...
    await expect(
      streamAuthoredAgentRun({ ...config, fetch: fetchMock }, 'assistant_bot', {
