...
export function createApiClient(config: ApiConfig) {
  ...
  return {
    ...
    // -- observability -------------------------------------------------------
    // NB: named `getObservabilityMetrics` (not `getMetrics`) — `getMetrics`
    // already serves the Prometheus `/metrics` text endpoint.
    ...
    // -- system (plain-text ops endpoints) -----------------------------------
    getHealth: (signal?: AbortSignal) => apiText(config, '/health', { signal }),
    getMetrics: (signal?: AbortSignal) => apiText(config, '/metrics', { signal }),
