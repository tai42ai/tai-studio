...
import { type ApiConfig, ApiError, ApiSchemaError } from '../index';
...
describe('hooks topic-verifier client transport', () => {
  ...
  it('setTopicVerifier PUTs the verifier binding to the topic-encoded route', async () => {
    const { client, captured } = harness(() =>
      jsonResponse({ data: { topic: 'events/new', verifier: 'github_hmac' } }),
    );
    const out = await client.setTopicVerifier('events/new', {
      verifier: 'github_hmac',
      config: { secret_env: 'GH_HOOK_SECRET' },
    });
    expect(captured[0]?.method).toBe('PUT');
    expect(captured[0]?.url).toBe('/api/hooks/topics/events%2Fnew/verifier');
    expect(captured[0]?.body).toEqual({
      verifier: 'github_hmac',
      config: { secret_env: 'GH_HOOK_SECRET' },
    });
    expect(out).toEqual({ topic: 'events/new', verifier: 'github_hmac' });
  });

  it('setTopicVerifier omits config from the body when none is provided', async () => {
    const { client, captured } = harness(() =>
      jsonResponse({ data: { topic: 'events', verifier: 'github_hmac' } }),
    );
    await client.setTopicVerifier('events', { verifier: 'github_hmac' });
  ...
  it('maps an unknown verifier name to a LOUD 400 ApiError', async () => {
    ...
    await expect(client.setTopicVerifier('events', { verifier: 'ghost' })).rejects.toBeInstanceOf(
      ApiError,
    );
  });
  ...
  it('deleteTopicVerifier DELETEs the topic-encoded route and parses the unbind', async () => {
    const { client, captured } = harness(() =>
      jsonResponse({ data: { removed: true, topic: 'events' } }),
    );
    const out = await client.deleteTopicVerifier('events');
    expect(captured[0]?.method).toBe('DELETE');
    expect(captured[0]?.url).toBe('/api/hooks/topics/events/verifier');
  ...
  it('maps an unbind with no binding to a LOUD 404 ApiError', async () => {
    ...
    await expect(client.deleteTopicVerifier('events')).rejects.toBeInstanceOf(ApiError);
  });
...
describe('hook listed-record contract', () => {
  it('parses a listed hook and the top-level topic → door map', async () => {
    const { client } = harness(() =>
      jsonResponse({
        data: { items: [hookRecord()], total: 1, trigger_auth: { 'events.created': 'verifier' } },
      }),
    );
    const out = await client.listHooks();
    expect(out.items[0]?.execution_key).toBe('svc-events');
    expect(out.trigger_auth).toEqual({ 'events.created': 'verifier' });
  });
  ...
  it('throws ApiSchemaError LOUDLY on an unknown door in the topic → door map', async () => {
    const { client } = harness(() =>
      jsonResponse({
        data: { items: [hookRecord()], total: 1, trigger_auth: { 'events.created': 'sso' } },
      }),
    );
    await expect(client.listHooks()).rejects.toBeInstanceOf(ApiSchemaError);
  });
