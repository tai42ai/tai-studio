...
describe('ComposeAgentDialog', () => {
  ...
  it('submits the base agent REGISTRATION name as base_tool (not tool_name, which can differ)', async () => {
    ...
    await waitFor(() => {
      expect(createPreset).toHaveBeenCalledWith(
        expect.objectContaining({ name: 'support_bot', base_tool: 'reg_name' }),
      );
    });
  });
...
describe('authored-agents list', () => {
  function authoredClient(over = {}) {
    return stubClient({
      ...
      listAgents: listOf(authorableAgent()),
      listPresets: () =>
        Promise.resolve([presetRecord({ name: 'support_bot', base_tool: 'authorable_agent' })]),
      ...over,
    });
  }
  ...
  it('lists an authored agent with a Manage link to its presets-page detail', async () => {
    ...
    const manage = within(row).getByRole('link', { name: /Manage authored agent support_bot/ });
    expect(manage).toHaveAttribute('href', '/presets?preset=support_bot');
  });
  ...
  it('resolves an authored agent whose base agent tool_name differs from its registration name', async () => {
    ...
    renderWithProviders(
      ...
      authoredClient({
        ...
        listPresets: () =>
          Promise.resolve([presetRecord({ name: 'support_bot', base_tool: 'reg_name' })]),
      }),
    );
    ...
    const getPreset = vi.fn(() =>
      Promise.resolve(
        presetDetail({
          name: 'support_bot',
          base_tool: 'authorable_agent',
          fixed_kwargs: { system_prompt: 'You are a helpdesk agent.' },
        }),
      ),
    );
  ...
  it('excludes a preset whose base_tool is a plain tool (not an agent) from the list', async () => {
    renderWithProviders(
      ...
      authoredClient({
        listPresets: () =>
          Promise.resolve([
            presetRecord({ name: 'support_bot', base_tool: 'authorable_agent' }),
    ...
    expect(rows[0]).toHaveAttribute('data-agent', 'support_bot');
    ...
    const getPreset = vi.fn(() =>
      Promise.resolve(
        presetDetail({
          name: 'support_bot',
          base_tool: 'authorable_agent',
          fixed_kwargs: { system_prompt: 'You are a helpdesk agent.', count: 3 },
        }),
      ),
    );
