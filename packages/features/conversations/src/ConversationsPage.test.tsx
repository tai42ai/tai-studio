...
describe('drill levels', () => {
  it('level 1: a bare page is the route picker', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Open route support' })).toBeInTheDocument();
  });

  it('level 2: ?route= is the route thread list with no transcript selected', async () => {
    renderWithProviders(<ConversationsPage search={{ route: 'support' }} />, {
      client: fullClient(),
    });
  ...
  it('level 3: ?route=&thread= opens the transcript beside the list', async () => {
    ...
    renderWithProviders(
      <ConversationsPage search={{ route: 'support', thread: 'svc-support/+15551234567' }} />,
      { client },
    );
...
describe('drill navigation', () => {
  it('walks routes to threads to transcript and back out again', async () => {
    ...
    await user.click(await screen.findByRole('link', { name: 'Open route support' }));
    expect(await screen.findByTestId('conversation-threads-table')).toBeInTheDocument();

    await user.click(
      await screen.findByRole('link', { name: 'Open thread svc-support/+15551234567' }),
    );
    expect(await screen.findByTestId('conversation-transcript')).toBeInTheDocument();
  ...
  it('moves focus to the pane it opened, and back to the row it came from', async () => {
    ...
    const routeRow = await screen.findByRole('link', { name: 'Open route support' });
    ...
    const threadRow = await screen.findByRole('link', {
      name: 'Open thread svc-support/+15551234567',
    });
    ...
    await waitFor(() => {
      expect(screen.getByRole('link', { name: 'Open route support' })).toHaveFocus();
    });
  });
  ...
  it('returns focus to the thread row when the transcript is closed', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('link', { name: 'Open thread svc-support/+15551234567' }),
      ).toHaveFocus();
    });
  });
  ...
  it('leaves focus alone on a deep link', async () => {
    renderWithLiveUrl((search) => <ConversationsPage search={search} />, {
      client: fullClient(),
      initialSearch: { route: 'support', thread: 'svc-support/+15551234567' },
    });
...
describe('a route change under the open pane', () => {
  ...
  it('does not announce the previous route as being back at its newest threads', async () => {
    ...
    const { rerender } = renderWithProviders(<ConversationsPage search={{ route: 'support' }} />, {
    ...
    rerender(<ConversationsPage search={{ route: 'billing' }} />);
  ...
  it('does not announce a page the previous route was still loading', async () => {
    ...
    const listConversationThreads = vi
      ...
      // The second page of `support` never lands: the route changes under it.
...
describe('URL self-repair', () => {
  ...
  it('keeps a real route when only the thread is blank', async () => {
    const client = fullClient();
    const { navigate } = renderWithProviders(
      <ConversationsPage search={{ route: 'support', thread: '   ' }} />,
      { client },
    );
    ...
    await waitFor(() => {
      expect(navigate).toHaveBeenCalledWith(
        'conversations',
        { route: 'support', thread: undefined },
        { replace: true },
      );
    });
    expect(navigate).toHaveBeenCalledTimes(1);
  ...
  it('leaves a legal search alone', async () => {
    const { navigate } = renderWithProviders(<ConversationsPage search={{ route: 'support' }} />, {
      client: fullClient(),
    });
