...
describe('ThreadList', () => {
  ...
  it('renders address, activity, count and delivery for each thread', async () => {
    ...
    expect(
      within(table).getByRole('link', { name: 'Open thread svc-support/+15551234567' }),
  ...
  it('marks the open thread as the current row', async () => {
    renderList(vi.fn().mockResolvedValue(page([makeThread()])), 'svc-support/+15551234567');

    const row = await screen.findByRole('link', { name: 'Open thread svc-support/+15551234567' });
  ...
  it('pages forward through a long route', async () => {
    ...
    const older = makeThread({ thread_id: 'svc-support/+15559999999', client_address: '+15559' });
    const listThreads = vi
      .fn()
