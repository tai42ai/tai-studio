/**
 * The `/system` feature page: operational health + Prometheus metrics + the worker
 * fleet.
 *
 * Health + metrics are plain-text skeleton ops endpoints (`/health`, `/metrics`)
 * read through `useApi()` and TanStack Query. The fleet section reads the backend
 ...
 * label, a `<CodeBlock>` child, a worker name), never through an HTML sink.
 */
...
import { backendInfoKey, fleetWorkersKey, healthKey, metricsKey, systemKindsKey } from './keys';
...
function MetricsCard(): ReactNode {
  const api = useApi();
  ...
  return (
    <Card>
      <div style={cardHeaderStyle}>
        <h2 style={titleStyle}>Metrics</h2>
        <Button
      ...
      {metrics.isPending ? (
        ...
        <EmptyState
          ...
          description="The metrics endpoint returned no data."
        />
      ) : (
        <CodeBlock code={metrics.data} language="Prometheus text" />
      )}
    </Card>
  );
}
