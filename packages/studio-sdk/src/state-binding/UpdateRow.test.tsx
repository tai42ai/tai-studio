...
describe('UpdateList', () => {
  ...
  it('seeds a new template update with a null adapter (never the empty string)', async () => {
    ...
    // At no point is an empty-content adapter stored (the platform refuses an empty adapter).
    expect(
      calls.some((call) => {
        const adapter = call[0]?.adapter;
        return (
          adapter !== null &&
          adapter !== undefined &&
  ...
  it('adds a custom update when no update jq is available', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Add update' }));
    expect(screen.getByRole('textbox', { name: 'Custom update jq 1' })).toBeInTheDocument();
  });
  ...
  it('switches a template update to custom and exposes the op-id field', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Custom update jq 1' })).toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: 'Op id' })).toBeInTheDocument();
  });
